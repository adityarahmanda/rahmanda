...
if ( ! function_exists( 'rahmanda_list_comments_callback' ) ) {
    function rahmanda_list_comments_callback($comment, $args, $depth) { 
    ?>
                            ...
                            <?php 
                                comment_reply_link(
                                    array_merge( 
                                        $args, 
                                        array(
                                            'reply_text'    => esc_html__( 'Balas', 'rahmanda' ),
                                            /* translators: Comment reply button text. %s: Comment author name. */
                                            'reply_to_text' => esc_html__( 'Balas ke %s', 'rahmanda'  ),
                                            'login_text'    => esc_html__( 'Balas', 'rahmanda'  ),
                                            'depth'         => $depth, 
...
} // End of if function_exists( 'rahmanda_list_comments_callback' ).

// filter default comment form fields.
add_filter('comment_form_default_fields', 'rahmanda_filter_default_comment_field');

if ( ! function_exists( 'rahmanda_filter_default_comment_field' ) ) {
	/**
	 ...
	 * @return array
	 */
    function rahmanda_filter_default_comment_field( $fields ) {
        ...
        $author = '<div class="comment-form-author col-md-6 form-name mb-3">'
            . '<input id="author" name="author" type="text" placeholder="' . sprintf( '%1$s', esc_html__('Nama', 'rahmanda') ) .'" maxlength="245" class="form-control" ' . $req . '>'
            . '</div>';

        $email = '<div class="comment-form-email col-md-6 mb-3">'
            . '<input id="email" name="email" type="text" placeholder="' . sprintf( '%1$s', esc_html__('Email', 'rahmanda') ) .'" maxlength="100" class="form-control" ' . $req . '>'
            . '</div>';
        ...
        $cookies = '<div class="comment-form-check form-check mb-3">'
            ...
            . '<label for="wp-comment-cookies">' . sprintf( '%1$s', esc_html__('Simpan nama dan email saya pada peramban ini untuk komentar saya berikutnya.', 'rahmanda') ) . '</label>'
            . '</div>';
...
} // End of if function_exists( 'rahmanda_filter_default_comment_field' ).

// Edit default WP's comment form.
add_filter('comment_form_defaults', 'rahmanda_comment_form_defaults');

if ( ! function_exists( 'rahmanda_comment_form_defaults' ) ) {
	/**
	 ...
	 * @return array
	 */
    function rahmanda_comment_form_defaults( $args, $post_id = null ) {
        if ( null === $post_id )
            $post_id = get_the_ID();
        else
            $id = $post_id;
        ...
        $comment_field = '<div class="comment-form-textarea mb-3">'
            . '<textarea id="comment" name="comment" placeholder="' . sprintf( '%1$s', esc_html__('Tuliskan komentarmu di sini', 'rahmanda') ) . '" maxlength="65525" class="form-control" aria-required="true" required></textarea>'
            . '</div>';

        $must_log_in = '<div class="must-log-in mb-3">' . 
            sprintf( 
                __( 'Kamu harus <a href="%1$s">masuk</a> terlebih dahulu untuk berkomentar.', 'rahmanda' ), 
...
} // End of if function_exists( 'rahmanda_comment_form_defaults' ).

// Add note if comments are closed.
add_action( 'comment_form_comments_closed', 'rahmanda_comment_form_comments_closed' );

if ( ! function_exists( 'rahmanda_comment_form_comments_closed' ) ) {
	/**
	 * Displays a note that comments are closed if comments are closed and there are comments.
	 */
	function rahmanda_comment_form_comments_closed() {
		if ( get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) {
			?>
			<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'rahmanda' ); ?></p>
			<?php
		}
	}

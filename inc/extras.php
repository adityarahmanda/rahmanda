...
if ( ! function_exists( 'understrap_more_link' ) ) {
	/**
	 * Add site info hook to WP hook library.
	 */
	function understrap_more_link() {
		do_action( 'understrap_more_link' );
	}
}

add_action( 'understrap_more_link', 'understrap_add_more_link' );
if ( ! function_exists( 'understrap_add_more_link' ) ) {
	/**
	 * Add site info content.
	 */
	function understrap_add_more_link() {
		$more_link = sprintf(
		...
		$more_link = '<div>'
			. '<a class="read-more-link" href="' . esc_url( get_permalink( get_the_ID() ) ) . '">' 
				. __( 'Baca Selengkapnya', 'understrap') . '<span class="screen-reader-text"> dari ' . get_the_title( get_the_ID() ) . '</span>'
			. '</a>'
		. '</div>';

		echo apply_filters( 'understrap_more_link_content', $more_link ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}